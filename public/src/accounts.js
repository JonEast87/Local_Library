...
function getBooksPossessedByAccount(account, books, authors) {
  // get account id
  const customerId = account.id
  let currentBooks = new Array()

  // use id to search for books checked out by said id
    // collect authorid and use it to search for author
    for (let i = 0; i < books.length; i++) {
      for (let j = 0; j < books[i].borrows.length; j++) {
        if (books[i].borrows[j].id === customerId 
    ...
    // create a new object containing the book and author info
